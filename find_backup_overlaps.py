...
db_path = "/media/lewis/1b/sqlitedb/telegram_backup.db"
...
# 1. FUZZY ALIAS LINKING via oldest footprints (catches renamed aliases like S / Sensitive Information 5)
    ...
    if len(backups) >= 2:
        ...
        for i in range(len(backups)):
            for j in range(i + 1, len(backups)):
                ...
                if b_contains_a:
                    # B fully contains the chronological span of A
                    cursor.execute("""
                        SELECT COUNT(*) 
                        FROM messages a 
                        WHERE a.chat_id = ? 
                          AND a.timestamp_unix BETWEEN ? AND ?
                          AND NOT EXISTS (
                              SELECT 1 
                              FROM messages b 
                              WHERE b.chat_id = ?
                                AND b.timestamp_unix BETWEEN a.timestamp_unix - 5 AND a.timestamp_unix + 5
                                AND (
                                    b.text = a.text 
                                    OR (a.text = '' AND b.text = '' AND b.media_path = a.media_path)
                                )
                          )
                    """, (a["chat_id"], a["min_unix"], a["max_unix"], b["chat_id"]))
                    missing_cnt = cursor.fetchone()[0]
                    
                    overlaps_found += 1
                    print(f"  -> Backup {letter_b} fully contains the chronological span of Backup {letter_a}!")
                    if missing_cnt == 0:
                        print(f"     SUCCESS: Backup {letter_b} has 0 missing messages from Backup {letter_a}! (Complete subset)")
                    else:
                        print(f"     WARNING: Backup {letter_b} is missing {missing_cnt} individual messages that exist in Backup {letter_a}'s range!")
                
                elif a_contains_b:
                    # A fully contains the chronological span of B
                    cursor.execute("""
                        SELECT COUNT(*) 
                        FROM messages a 
                        WHERE a.chat_id = ? 
                    ...
                    """, (b["chat_id"], b["min_unix"], b["max_unix"], a["chat_id"]))
                    missing_cnt = cursor.fetchone()[0]
                    
                    overlaps_found += 1
                    print(f"  -> Backup {letter_a} fully contains the chronological span of Backup {letter_b}!")
                    if missing_cnt == 0:
                        print(f"     SUCCESS: Backup {letter_a} has 0 missing messages from Backup {letter_b}! (Complete subset)")
                    else:
                        print(f"     WARNING: Backup {letter_a} is missing {missing_cnt} individual messages that exist in Backup {letter_b}'s range!")
                    ...
                    if a["min_unix"] is not None and a["max_unix"] is not None and b["min_unix"] is not None and b["max_unix"] is not None:
                        ...
                        if overlap_start <= overlap_end:
                            ...
                            # Let's count missing messages in the overlapping region
                            # A's messages in overlap missing from B
                            cursor.execute("""
                                SELECT COUNT(*) 
                                FROM messages a 
                                WHERE a.chat_id = ? 
                            ...
                            """, (a["chat_id"], overlap_start, overlap_end, b["chat_id"]))
                            missing_a_in_b = cursor.fetchone()[0]
                            
                            # B's messages in overlap missing from A
                            cursor.execute("""
                                SELECT COUNT(*) 
                                FROM messages a 
                                WHERE a.chat_id = ? 
                            ...
                            """, (b["chat_id"], overlap_start, overlap_end, a["chat_id"]))
                            missing_b_in_a = cursor.fetchone()[0]
                            ...
                            if missing_a_in_b == 0 and missing_b_in_a == 0:
                                ...
                                if missing_a_in_b > 0:
                                    print(f"     WARNING: Backup {letter_b} is missing {missing_a_in_b} messages from Backup {letter_a} in the overlapping span.")
                                if missing_b_in_a > 0:
                                    print(f"     WARNING: Backup {letter_a} is missing {missing_b_in_a} messages from Backup {letter_b} in the overlapping span.")
