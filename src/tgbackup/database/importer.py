...
def ensure_search_schema(conn: sqlite3.Connection) -> None:
    """Create incremental-search indexes/triggers without rebuilding an existing FTS."""
    cursor = conn.cursor()
    ...
    fts_columns = {
        str(row[1]) for row in cursor.execute("PRAGMA table_info(messages_fts)").fetchall()
    }
    ...
    existing_triggers = {
        str(row[0]) for row in cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='trigger'"
        )
    }
    if "messages_ai" not in existing_triggers:
    ...
    if "messages_ad" not in existing_triggers:
        cursor.executescript(
            """
    ...
    if "messages_au" not in existing_triggers:
        cursor.executescript(
            """
...
def setup_database(db_path: str) -> sqlite3.Connection:
    ...
    for column in (
        ...
        try:
            cursor.execute(f"ALTER TABLE chats ADD COLUMN {column} TEXT;")
        except sqlite3.OperationalError:
            pass
    for column, sql_type in (
        ...
        try:
            cursor.execute(f"ALTER TABLE chats ADD COLUMN {column} {sql_type};")
        except sqlite3.OperationalError:
            pass

    # Create Messages table with a stable chat/message uniqueness constraint for upserts.
    cursor.execute("""
    ...
    existing_columns = {
        str(row[1]) for row in cursor.execute("PRAGMA table_info(messages)").fetchall()
    }
    for column, sql_type in archival_columns.items():
        if column not in existing_columns:
            cursor.execute(f"ALTER TABLE messages ADD COLUMN {column} {sql_type}")
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS backup_sources (
            ...
            imported_unix INTEGER NOT NULL,
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS backup_import_files (
            ...
            skipped_records INTEGER NOT NULL DEFAULT 0,
            completed_unix INTEGER NOT NULL,
            PRIMARY KEY(source_key, original_path),
            FOREIGN KEY(source_key) REFERENCES backup_sources(source_key)
        )
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS message_sources (
            ...
            message_id INTEGER NOT NULL,
            source_key TEXT NOT NULL,
            PRIMARY KEY(chat_id, message_id, source_key),
            FOREIGN KEY(source_key) REFERENCES backup_sources(source_key)
        )
    """)
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_messages_source_key ON messages(source_key)")
